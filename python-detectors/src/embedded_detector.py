...
from iml_detectors.detector_registry import DetectorRegistry
...
def detect(frame: np.ndarray, frame_id: int | None = None, product_type: str | None = None,
           rois: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    pt = product_type or _DEFAULT_PRODUCT_TYPE
    detector = _REGISTRY.resolve(_ACTIVE_DETECTOR)
    try:
        result = detector.detect(frame, rois=rois, product_type=pt, threshold=None, include_visuals=False)
        if isinstance(result, dict):
            ...
            return result
        return {"ok": False, "status": "ERROR", "anomaly_score": 1.0, "message": "detector_result_not_dict"}
    except Exception as exc:
        return {
            "ok": False,
            "status": "ERROR",
            "anomaly_score": 1.0,
            "message": f"inspect_error:{exc}",
        }
